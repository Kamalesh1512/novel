...
export const AnimatedHeading = ({ children }: any) => {
...
const Testimonials = () => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setCurrentOffset((prev) => {
        const cardHeight = 220; // avg card height including gap
        const totalHeight = testimonials.length * cardHeight;

        const newOffset = prev - 1.5; // scroll speed
  ...
  return (
    <div
      className="max-w-8xl mx-auto overflow-hidden rounded-t-2xl"
      style={{
    ...
    >
      <div>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-start lg:items-center">
          ...
          {/* Right Column - Fixed Height Carousel */}
          <div
            className="relative h-64 sm:h-80 lg:h-96 overflow-y-visible"
            ...
            >
              {duplicatedTestimonials.map((testimonial, index) => (
                <div key={`${testimonial.id}-${index}`} className="mb-6">
                  <div className="bg-white/20 backdrop-blur-md rounded-xl sm:rounded-2xl p-4 sm:p-6 lg:p-8 shadow-lg border border-white/30 hover:shadow-xl transition-shadow min-h-[200px] lg:min-h-[220px] flex flex-col justify-between">
